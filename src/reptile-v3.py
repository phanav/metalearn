...
dataname = "resisc45"
resultprefix = "resisc45-reptile-nopretrain"
...
figdir = joinpath(resultdir, dataname, 'fig')
modeldir = joinpath(resultdir, dataname, 'model')
metricdir = joinpath(resultdir, dataname, 'metric')
...
np.random.permutation("a b c".split())
...
def init_assess_record(filepath, nway):
...
def train_reptile(params, meta_optim, meta_model, lossfunc, metatrain_classes, metatest_classes,
    ...
    init_assess_record(assess_path, params['nway'])
    assess_iteration = 0
...
model_name = resultprefix + '-resnet_nopretrain-way{}-shot{}'.format(params['nway'], params['kshot'])
